...
@dynamic eventId, senderId, senderDisplayName, text, isMediaMessage;
...
- (NSUInteger)messageHash {
    ...
    for (int i = 0; i < [self.objectId length]; i++) {
        int ascii = [self.objectId characterAtIndex:i];
        NSString *asciiString = [NSString stringWithFormat:@"%dl", ascii];
        [hashString stringByAppendingString:asciiString];
    }
    return (NSUInteger)[hashString integerValue];
    */
